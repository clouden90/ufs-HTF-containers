                    ...
                    if(obs_compare and profiles_mean['vars'][k] in obs_dict):
                        ...
                        if(bias_val):
                            ...
                            for i in range(len(scm_datasets)):
                                ...
                                mae_data.append(np.mean(np.abs(interp_values[0:] - obs_mean_data[0:]))) 
                            if (label == 'T (K)'):
                            	print(label,mae_data);diff = np.abs(mae_data[1]-mae_data[0])/mae_data[0]
                            	print(label,diff)
                            	try:
                                	if (diff > float(rel_tol)):
                                		raise Exception('!!!Warning!!! diff is larger than user-specified rel_tol, STOP!!!')
